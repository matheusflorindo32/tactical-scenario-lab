<x-layouts.app :current="'dashboard'" :title="'Painel executivo · Tactical Scenario Lab'">
    <x-slot:breadcrumbs>
        <x-breadcrumb :items="[
    ...
    <x-slot:header>
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                ...
                <p class="mt-1.5 max-w-3xl text-sm leading-6 text-ink-500">Indicadores derivados de execuções e avaliações M4. Resultados históricos sem classificação não são convertidos artificialmente em aprovação ou reprovação.</p>
            </div>
            <x-button href="{{ route('dashboard') }}" variant="secondary">Voltar ao painel do instrutor</x-button>
        </div>
    </x-slot:header>

    <form method="GET" action="{{ route('dashboard.executive') }}" class="mb-6 grid gap-3 rounded-xl border border-stone-200 bg-white p-4 shadow-sm sm:grid-cols-3 lg:grid-cols-4">
        <div>
    ...
    <section aria-label="Indicadores executivos" class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
    ...
    <div class="mt-8 grid gap-6 lg:grid-cols-2">
        <x-card title="Erros críticos observados" subtitle="Ocorrências reais, não catálogo do cenário" accent="emergency">
            @if ($top_observed_errors->isEmpty())
                <p class="text-sm text-ink-500">Nenhum erro crítico observado no período.</p>
            @else
                <ol class="space-y-3">
        ...
        <x-card title="Tendência mensal" subtitle="Execuções dentro do período selecionado" accent="navy">
            @if ($monthly_trend->isEmpty())
                <p class="text-sm text-ink-500">Sem execuções para compor tendência.</p>
            @else
                <div class="space-y-3">
