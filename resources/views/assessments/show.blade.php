...
$assessmentState = $assessment->isFinalized() ? 'finalized' : 'draft';
$sectionItems = [
    ['label' => 'Resumo', 'href' => '#summary', 'state' => 'current'],
    ['label' => 'Rubrica', 'href' => '#rubric'],
    ['label' => 'Erros críticos', 'href' => '#critical-errors'],
    ['label' => 'Tempos-chave', 'href' => '#key-times'],
    ['label' => 'Debrief', 'href' => '#debrief'],
    ['label' => 'Plano de ação', 'href' => '#action-plan'],
];
if ($isEditable) {
    $sectionItems[] = ['label' => 'Finalizar', 'href' => '#finalize', 'state' => 'attention'];
}
@endphp
...
<x-layouts.app :current="'scenarios'" :title="'Avaliação da execução ' . $execution->sequence_number . ' · Tactical Scenario Lab'">
    ...
    <div class="mb-6">
        <x-section-nav :items="$sectionItems" label="Navegação da avaliação" />
    </div>

    @if ($assessment->isFinalized())
        <div class="mb-6">
            <x-alert variant="success" title="Conteúdo histórico congelado">
                Esta avaliação foi finalizada e seu conteúdo institucional não pode mais ser reescrito. Apenas o status operacional das ações do plano pode evoluir nos fluxos autorizados.
            </x-alert>
        </div>
    @endif

    <div data-assessment-state="{{ $assessmentState }}" class="space-y-6">
        <section id="summary" class="scroll-mt-24" aria-labelledby="assessment-summary-heading">
            <x-card title="Resumo da avaliação" subtitle="Composição transparente e reproduzível do resultado" accent="navy">
                <h2 id="assessment-summary-heading" class="sr-only">Resumo da avaliação</h2>
        ...
        <div class="grid gap-6 xl:grid-cols-2">
            <section id="rubric" class="scroll-mt-24" aria-labelledby="assessment-rubric-heading">
                <x-card title="Rubrica" subtitle="Critérios ponderados, notas e evidências objetivas" accent="clinical">
                    <h2 id="assessment-rubric-heading" class="sr-only">Rubrica e evidências</h2>
                    <div class="space-y-4">
            ...
            <section id="critical-errors" class="scroll-mt-24" aria-labelledby="assessment-critical-errors-heading">
                <x-card title="Erros críticos observados" subtitle="Catálogo da versão separado das ocorrências desta execução" accent="alert">
                    <h2 id="assessment-critical-errors-heading" class="sr-only">Erros críticos observados</h2>
                    <div class="space-y-3">
        ...
        <section id="key-times" class="scroll-mt-24" aria-labelledby="assessment-key-times-heading">
            <x-card title="Tempos-chave" subtitle="Marcos temporais calculados pelo servidor a partir do início da execução" accent="navy">
                <h2 id="assessment-key-times-heading" class="sr-only">Tempos-chave</h2>
        ...
        <section id="debrief" class="scroll-mt-24" aria-labelledby="assessment-debrief-heading">
            <div class="mb-4">
                <p class="text-[11px] font-semibold uppercase tracking-[0.16em] text-ink-500">Análise pós-execução</p>
                <h2 id="assessment-debrief-heading" class="mt-1 font-display text-xl font-semibold text-navy-950">Debrief estruturado</h2>
                <p class="mt-1 text-sm leading-6 text-ink-500">Separe fatos observáveis, interpretação profissional e recomendações para manter o registro auditável.</p>
            </div>
        ...
        <section id="action-plan" class="scroll-mt-24" aria-labelledby="assessment-action-plan-heading">
            <x-card title="Plano de ação" subtitle="Ações corretivas com responsável, prazo e acompanhamento operacional" accent="alert">
                <h2 id="assessment-action-plan-heading" class="sr-only">Plano de ação</h2>
                <div class="space-y-4">
        ...
        @if ($isEditable)
            <section id="finalize" class="scroll-mt-24" aria-labelledby="assessment-finalize-heading">
                <x-card title="Prontidão para finalização" subtitle="A finalização congela o conteúdo histórico da avaliação" accent="navy">
                    <h2 id="assessment-finalize-heading" class="sr-only">Prontidão para finalização</h2>
                    ...
                    <div class="mt-4 rounded-lg border border-alert-100 bg-alert-50 p-4">
                        <p class="text-xs font-semibold uppercase tracking-[0.12em] text-alert-700">Ação irreversível para o conteúdo histórico</p>
                        <p class="mt-1 text-sm leading-6 text-ink-700">Após finalizar, rubrica, evidências, erros, tempos, debrief e conteúdo das ações ficam congelados pelos invariantes institucionais.</p>
                    </div>
