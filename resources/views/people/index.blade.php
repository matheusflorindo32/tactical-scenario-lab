<x-layouts.app :current="'people'" :title="'Pessoas · Tactical Medicine Academy'">
    ...
    <form method="GET" action="{{ route('people.index') }}" class="mb-6 grid gap-3 rounded-2xl border border-ink-200 bg-white p-4 shadow-sm sm:grid-cols-[1fr_220px_180px_auto]">
        <label>
            <span class="sr-only">Buscar pessoa</span>
            <input name="q" value="{{ $search }}" class="w-full rounded-xl border border-ink-300 px-4 py-3 text-sm outline-none focus:border-navy-700 focus:ring-4 focus:ring-navy-100" placeholder="Nome ou nome social">
        </label>
        <label>
    ...
    <section class="overflow-hidden rounded-2xl border border-ink-200 bg-white shadow-sm">
        ...
        @empty
            <div class="px-6 py-16 text-center">
                ...
                <p class="mx-auto mt-2 max-w-md text-sm leading-6 text-ink-500">Ajuste os filtros ou faça um cadastro rápido sem exigir documentos.</p>
