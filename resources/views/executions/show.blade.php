...
<x-layouts.app :current="'scenarios'" :title="'Execução ' . $execution->sequence_number . ' · Tactical Scenario Lab'">
        ...
        <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div class="min-w-0 flex-1">
                ...
                <p class="mt-2 max-w-3xl text-sm leading-6 text-ink-500">
                    {{ $execution->scenarioVersion->scenario->title }} · coordenação operacional, pessoas, recursos, injects e timeline em um único contexto de execução.
                </p>
            </div>
    ...
    @if ($errors->any())
        <div class="mb-6">
            <x-alert variant="danger" title="Revise os dados enviados">
                {{ $errors->first() }}
            </x-alert>
        </div>
    @endif
            ...
            <x-card title="Situação da execução" subtitle="Contexto operacional congelado na versão publicada" accent="navy">
                <dl class="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 font-display text-2xl font-semibold tabular-nums text-navy-950">
                            {{ number_format((int) $execution->scenarioVersion->estimated_casualty_count, 0, ',', '.') }}
                        </dd>
                    </div>
                    <div>
                ...
                <div class="mt-6 grid gap-4 border-t border-stone-100 pt-5 sm:grid-cols-3">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.12em] text-ink-500">Início</p>
                        <p class="mt-1 text-sm font-medium text-ink-800">{{ $execution->started_at?->format('d/m/Y H:i:s') ?? 'Ainda não iniciada' }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.12em] text-ink-500">Conclusão</p>
                        <p class="mt-1 text-sm font-medium text-ink-800">{{ $execution->completed_at?->format('d/m/Y H:i:s') ?? '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.12em] text-ink-500">Mecanismo</p>
                        <p class="mt-1 text-sm font-medium text-ink-800">{{ $execution->scenarioVersion->mechanism }}</p>
                    </div>
                </div>
            </x-card>
            ...
            <x-card title="Timeline da execução" subtitle="Registro cronológico append-only; eventos históricos não são reescritos" accent="navy">
                ...
                @else
                    <ol class="space-y-4">
                        @foreach ($execution->events as $event)
                            <li class="relative rounded-lg border border-stone-200 bg-white px-4 py-4 shadow-sm">
                                    ...
                                    <div>
                                        ...
                                        @if ($event->team || $event->participant)
                                            <p class="mt-1 text-xs text-ink-500">
                                                {{ $event->team?->label }}
                                                @if ($event->team && $event->participant) · @endif
                                                {{ $event->participant?->person?->preferredName() }}
                                            </p>
                                        @endif
                                    </div>
                                </div>
                ...
                @if ($canManage && $execution->isRunning())
                    <form method="POST" action="{{ route('execution-events.store', $execution) }}" class="mt-6 grid gap-4 border-t border-stone-100 pt-5 md:grid-cols-2">
                        ...
                        <div class="md:col-span-2">
                            <x-button type="submit">Adicionar à timeline</x-button>
                        </div>
                    </form>
                @endif
            </x-card>
                        ...
                        <div class="mt-3 space-y-2">
                            @forelse ($execution->teams as $team)
                                <div class="rounded-lg border border-stone-200 bg-stone-50 px-4 py-3">
                                    ...
                                    @if ($team->description)
                                        <p class="mt-1 text-xs leading-5 text-ink-500">{{ $team->description }}</p>
                                    @endif
                            ...
                            @forelse ($execution->participants as $participant)
                                <div class="rounded-lg border border-stone-200 bg-stone-50 px-4 py-3">
                                    ...
                                    <p class="mt-1 text-xs text-ink-500">
                                        ...
                                        @if ($participant->team) · {{ $participant->team->label }} @endif
                                    </p>
                                </div>
                            @empty
                        ...
                        <form method="POST" action="{{ route('execution-participants.store', $execution) }}" class="space-y-3">
                            ...
                            <select name="person_uuid" class="w-full rounded-md border-stone-300 text-sm focus:border-navy-500 focus:ring-navy-500" required>
                                ...
                                @foreach ($people as $person)
                                    <option value="{{ $person->uuid }}">{{ $person->preferredName() }}</option>
                                @endforeach
                            </select>
                            <select name="execution_team_uuid" class="w-full rounded-md border-stone-300 text-sm focus:border-navy-500 focus:ring-navy-500">
                                ...
                                @foreach ($execution->teams as $team)
                                    <option value="{{ $team->uuid }}">{{ $team->label }}</option>
                                @endforeach
                            </select>
                    ...
                    @forelse ($execution->resources as $resource)
                        <div class="rounded-lg border border-stone-200 bg-stone-50 p-4">
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <x-badge :variant="$resource->status === 'available' ? 'clinical' : ($resource->status === 'depleted' ? 'emergency' : 'neutral')" size="sm">
                                    {{ $resource->status }}
                                </x-badge>
                            </div>
                            <p class="mt-2 text-xs tabular-nums text-ink-500">
                                Planejado {{ $resource->planned_quantity }} · disponível {{ $resource->available_quantity }} · usado {{ $resource->used_quantity }}
                            </p>
                            ...
                            @if ($canManage && $execution->canConfigure())
                                <form method="POST" action="{{ route('execution-resources.update', $resource) }}" class="mt-3 grid grid-cols-3 gap-2">
                                    ...
                                    <select name="status" aria-label="Estado de {{ $resource->name }}" class="col-span-2 rounded-md border-stone-300 text-sm focus:border-navy-500 focus:ring-navy-500">
                                        <option value="available" @selected($resource->status === 'available')>Disponível</option>
                                        <option value="unavailable" @selected($resource->status === 'unavailable')>Indisponível</option>
                                        <option value="depleted" @selected($resource->status === 'depleted')>Esgotado</option>
                                    </select>
                    ...
                    @forelse ($execution->injects as $inject)
                        <div class="rounded-lg border border-stone-200 bg-white p-4 shadow-sm">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    <p class="text-sm font-semibold text-ink-900">{{ $inject->label }}</p>
                                    <p class="mt-1 text-xs leading-5 text-ink-500">{{ $inject->content }}</p>
                                </div>
                                <x-badge :variant="$inject->status === 'delivered' ? 'clinical' : ($inject->status === 'cancelled' ? 'neutral' : 'alert')" size="sm">
                                    {{ $inject->status }}
                                </x-badge>
                            </div>
                            ...
                            @if ($canManage && $inject->isPlanned())
                                <div class="mt-3 flex flex-wrap gap-2">
                                    @if ($execution->isRunning())
                                        <form method="POST" action="{{ route('execution-injects.deliver', $inject) }}">
                                            @csrf
                                            @method('PATCH')
                                            <x-button type="submit" size="sm">Entregar inject</x-button>
                                        </form>
                                    @endif
                                    @if ($execution->canConfigure())
                                        <form method="POST" action="{{ route('execution-injects.cancel', $inject) }}">
                                            @csrf
                                            @method('PATCH')
                                            <x-button type="submit" size="sm" variant="ghost">Cancelar</x-button>
                                        </form>
                                    @endif
                                </div>
                            @endif
