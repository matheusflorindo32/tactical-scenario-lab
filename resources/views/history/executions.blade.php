<x-layouts.app :current="'dashboard'" :title="'Histórico de execuções · Tactical Scenario Lab'">
    <x-slot:breadcrumbs>
        <x-breadcrumb :items="[
    ...
    <x-card title="Execuções" subtitle="Mais recentes primeiro" padding="none">
        @if ($executions->isEmpty())
            <div class="p-6"><x-empty-state icon="clip" title="Nenhuma execução encontrada" description="Ajuste o período ou os filtros para consultar outro recorte institucional." /></div>
        @else
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-stone-200 text-sm">
                    <thead class="bg-stone-50 text-left text-xs font-semibold uppercase tracking-[0.08em] text-ink-500">
                        <tr>
                            <th class="px-5 py-3">Execução</th>
                            <th class="px-5 py-3">Unidades históricas</th>
                            <th class="px-5 py-3">Estado</th>
                            <th class="px-5 py-3">Avaliação</th>
                            <th class="px-5 py-3">Erros</th>
                            <th class="px-5 py-3">Ações abertas</th>
                        </tr>
                    </thead>
            ...
            <div class="border-t border-stone-100 px-5 py-4">{{ $executions->links() }}</div>
        @endif
    </x-card>
</x-layouts.app>
