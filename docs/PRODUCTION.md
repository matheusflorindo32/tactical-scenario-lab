...
This document is the M6 production operating contract for Tactical Scenario Lab. It describes the deployment posture that matches the tested application behavior. It is not a substitute for provider-specific infrastructure documentation or an incident-response plan.
...
1. Install the immutable application build/artifact.
2. Configure production environment and secrets outside source control.
3. Run `php artisan production:preflight` and stop the deployment on any violation.
4. Run `php artisan migrate --force` using the migration credentials.
5. Warm/cache Laravel configuration and routes where the deployment platform uses those caches.
6. Switch web/queue processes to the least-privilege runtime database credentials.
7. Verify `GET /health/live`, then `GET /health/ready`.
8. Admit production traffic only after readiness is successful.
...
M6 adds PostgreSQL protections that are intentionally narrower than application business logic:
...
The current deployment supports database-backed session, cache and queue storage at the intended scale. Redis is optional and is not an M6 production requirement.
...
- queue workers, if enabled, are running the intended release;
- backup/PITR monitoring is healthy according to the infrastructure provider.

A production deployment is not considered complete merely because migration commands returned zero. Readiness, runtime-role separation and recovery posture are part of the operating contract.
